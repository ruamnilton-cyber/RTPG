...
export function isPlatformAdmin(user?: RequestUser) {
  return user?.role === "ADMIN" && user.email === "admin@rtpg.local";
}
...
export function requirePlatformAdmin(req: Request, res: Response, next: NextFunction) {
  ...
  if (!isPlatformAdmin(req.user)) {
    return res.status(403).json({ message: "Recurso exclusivo do administrador da plataforma." });
  }
