...
import type { FinanceTitle } from "../contracts/platform";
import { getFinanceTitles } from "../services/platform";

const router = Router();
router.use(requireAuth, requireBar);

function matchFinanceTitleToBar(title: FinanceTitle, barId: string, barCode: string) {
  if (title.branchId === barId || title.branchId === barCode) {
    return true;
  }
  if (title.branchId === "branch-main" || !title.branchId) {
    return barCode === "principal";
  }
  return false;
}
...
router.get("/dashboard", async (req, res) => {
  ...
  const manualRevenue = financeTitles
    .filter((title) => {
      if (title.kind !== "RECEBER" || title.status === "CANCELADO") {
        return false;
      }
      if (!matchFinanceTitleToBar(title, req.barId!, barCode)) {
        return false;
      }
      const referenceDate = new Date(title.dueDate || title.createdAt);
      return referenceDate >= today.start && referenceDate <= today.end;
    })
    .reduce((sum, title) => sum + Number(title.amount), 0);
...
router.get("/dre", async (req, res) => {
  ...
  const [sales, expenses, previousSales, previousExpenses, financeTitles] = await Promise.all([
    prisma.sale.findMany({
  ...
  const previousManualRevenue = financeTitles
    .filter((title) => {
      if (title.kind !== "RECEBER" || title.status === "CANCELADO") {
        return false;
      }
      ...
      const referenceDate = new Date(title.dueDate || title.createdAt);
      return referenceDate >= previousRange.start && referenceDate <= previousRange.end;
    })
    .reduce((sum, title) => sum + Number(title.amount), 0);
