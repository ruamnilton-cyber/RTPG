        ...
        <section className="flex flex-col justify-center p-8 md:p-10">
          ...
          {tab === "lead" && !leadSent && (
            <form onSubmit={handleLead} className="space-y-4">
              ...
              <label className="flex items-start gap-3 rounded-2xl border border-gray-200 p-3 cursor-pointer">
                <input type="checkbox" name="termos" required className="mt-0.5 shrink-0" />
                <span className="text-xs text-gray-500 leading-relaxed">
                  Li e aceito os{" "}
                  <Link to="/termos" target="_blank" className="underline text-amber-700 hover:text-amber-900">
                    Termos de Uso e Contrato de Serviço
                  </Link>
                  . Entendo que o acesso é disponibilizado após contato da equipe RTPG.
                </span>
              </label>
              {error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
              <button className="btn-primary w-full" disabled={loading}>
