...
import { ensureOrderForTableSession, markOrderClosedFromSale, syncOrderItemsFromSession } from "../services/orders";
...
async function ensureSessionOrderIfNeeded(sessionId: string) {
  ...
  if (!session || session.order || !session.table) {
    return;
  }

  await prisma.$transaction(async (tx) => {
    const order = await ensureOrderForTableSession(tx, {
    ...
    await syncOrderItemsFromSession(tx, session.id);
  });
}

router.get("/tables", async (req, res) => {
  const orphanSessions = await prisma.tableSession.findMany({
    where: {
      table: { barId: req.barId! },
      status: { in: ["ABERTA", "AGUARDANDO_FECHAMENTO"] },
      orderId: null
    },
    select: { id: true }
  });
  ...
  for (const session of orphanSessions) {
    await ensureSessionOrderIfNeeded(session.id);
  }
...
router.delete("/tables/:tableId", requireRole("ADMIN"), async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.waiterCall.deleteMany({ where: { tableId: table.id } });
    await tx.restaurantTable.delete({ where: { id: table.id } });

    const remainingTables = await tx.restaurantTable.findMany({
      where: {
        barId: req.barId!,
        number: { gt: table.number }
      },
      orderBy: { number: "asc" }
    });

    for (const currentTable of remainingTables) {
      const nextNumber = currentTable.number - 1;
      const normalizedName = currentTable.name.trim();
      const nextName =
        normalizedName === currentTable.name && /^Mesa\s+\d+$/i.test(normalizedName)
          ? `Mesa ${nextNumber}`
          : /^Mesa\s+\d+$/i.test(normalizedName)
            ? `Mesa ${nextNumber}`
            : currentTable.name;
...
router.post("/tables/:tableId/session", async (req, res) => {
  ...
  if (existing) {
    await ensureSessionOrderIfNeeded(existing.id);
    const refreshedExisting = await prisma.tableSession.findUnique({
    ...
    return res.json(refreshedExisting);
  }
  ...
  const session = await prisma.$transaction(async (tx) => {
    ...
    await tx.restaurantTable.update({ where: { id: req.params.tableId }, data: { status: "OCUPADA" } });
    return created;
  });
...
router.get("/tables/:tableId/session", async (req, res) => {
  ...
  const orphanSession = await prisma.tableSession.findFirst({
    where: {
      tableId: req.params.tableId,
      status: { in: ["ABERTA", "AGUARDANDO_FECHAMENTO"] },
      orderId: null
  ...
  if (orphanSession) {
    await ensureSessionOrderIfNeeded(orphanSession.id);
  }
...
router.post("/sessions/:sessionId/items", async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    await syncOrderItemsFromSession(tx, req.params.sessionId);
  });
...
router.put("/session-items/:itemId", async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    await syncOrderItemsFromSession(tx, current.tableSessionId);
  });
...
router.post("/sessions/:sessionId/close", async (req, res) => {
  const data = z.object({
    ...
    paymentMethod: z.enum(["PIX", "CREDITO", "DEBITO", "DINHEIRO", "OUTRO"]).optional()
  }).parse(req.body);
  const session = await prisma.tableSession.findUnique({
    where: { id: req.params.sessionId },
    include: {
  ...
  const sale = await prisma.$transaction(async (tx) => {
    ...
    await markOrderClosedFromSale(tx, {
      sessionId: session.id,
