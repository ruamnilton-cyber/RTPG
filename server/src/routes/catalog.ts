...
import { applyMenuImport, parseMenuText } from "../services/menu-importer";
import { extractMenuTextFromUpload } from "../services/menu-ocr";
...
router.post("/menu-import/preview", requireRole("ADMIN"), async (req, res) => {
  const data = z.object({
    rawText: z.string().min(5, "Cole um cardapio com pelo menos algumas linhas para importar.")
  }).parse(req.body);

  res.json(parseMenuText(data.rawText));
});

router.post("/menu-import/preview-file", requireRole("ADMIN"), async (req, res) => {
  const data = z.object({
  ...
  const extractedText = await extractMenuTextFromUpload(data);
  const parsed = parseMenuText(extractedText);
  res.json({ ...parsed, texto_extraido: extractedText });
});

router.post("/menu-import/apply", requireRole("ADMIN"), async (req, res) => {
  const data = z.object({
    parsed: z.any()
  }).parse(req.body);

  const result = await applyMenuImport(req.barId!, data.parsed, req.user?.userId);
  res.json(result);
});
