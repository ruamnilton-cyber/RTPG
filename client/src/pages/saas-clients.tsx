...
export function SaasClientsPage() {
  ...
  const [billingConfig, setBillingConfig] = useState<BillingAdminConfig | null>(null);
  const [billingConfigMessage, setBillingConfigMessage] = useState("");

  async function load(nextSearch = "") {
    const [list, summary, config] = await Promise.all([
      ...
      apiRequest<BillingAdminConfig>("/auth/billing-admin/config", { token })
    ]);
    setClients(list);
    setOverview(summary);
        ...
        <div className="space-y-5">
          <form
            ...
            onSubmit={async (event: FormEvent<HTMLFormElement>) => {
              ...
              setBillingConfigMessage("");
              const formData = new FormData(event.currentTarget);
              const saved = await apiRequest<BillingAdminConfig>("/auth/billing-admin/config", {
                method: "PUT",
                token,
                body: {
                  pixKey: String(formData.get("pixKey") ?? ""),
                  pixRecipientName: String(formData.get("pixRecipientName") ?? ""),
                  pixCity: String(formData.get("pixCity") ?? ""),
                  creditCardCheckoutUrl: String(formData.get("creditCardCheckoutUrl") ?? ""),
                  appBaseUrl: String(formData.get("appBaseUrl") ?? ""),
                  webhookSecret: String(formData.get("webhookSecret") ?? "")
                }
              });
              setBillingConfig(saved);
              setBillingConfigMessage("Configuracoes de cobranca salvas com sucesso.");
              await load(search);
            }}
          >
            <div>
              ...
              <h3 className="mt-2 text-2xl font-bold">Configuracao de cobranca</h3>
              <p className="mt-1 text-sm text-muted">
                Defina aqui a chave Pix e o checkout de cartao para todos os restaurantes pagarem assinatura.
              </p>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-1 md:col-span-2">
                <span className="label">Chave Pix de recebimento</span>
                <input className="input" name="pixKey" placeholder={billingConfig?.pixKeyMasked || "email, telefone ou chave aleatoria"} />
              </label>
              <label className="space-y-1">
                <span className="label">Nome do recebedor no Pix</span>
                <input className="input" name="pixRecipientName" defaultValue={billingConfig?.pixRecipientName || ""} />
              </label>
              <label className="space-y-1">
                <span className="label">Cidade do Pix</span>
                <input className="input" name="pixCity" defaultValue={billingConfig?.pixCity || ""} />
              </label>
              <label className="space-y-1 md:col-span-2">
                <span className="label">URL checkout cartao (com placeholders)</span>
                <input
                  className="input"
                  name="creditCardCheckoutUrl"
                  defaultValue={billingConfig?.creditCardCheckoutUrl || ""}
                  placeholder="https://seu-gateway.com/checkout?amount={amount}&client={client_id}"
                />
              ...
              <label className="space-y-1 md:col-span-2">
                ...
                <input
                  className="input"
                  name="appBaseUrl"
                  defaultValue={billingConfig?.appBaseUrl || ""}
                  placeholder="https://app.seudominio.com"
                />
              </label>
              <label className="space-y-1 md:col-span-2">
                <span className="label">Segredo do webhook de pagamento</span>
                <input className="input" name="webhookSecret" type="password" placeholder="Segredo para validar notificacoes" />
              </label>
            </div>
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-2xl p-3 surface-soft text-xs">Pix: {billingConfig?.pixConfigured ? "configurado" : "pendente"}</div>
